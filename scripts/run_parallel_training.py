...
def load_and_resolve(args):
    ...
    experiment_name = f"{base_name}_smoke" if args.smoke and not args.experiment_name else base_name
...
def validate_files(resolved):
    ...
    weaver = PROJECT_DIR / ".venv/bin/weaver"
    ...
    if missing_samples:
        raise FileNotFoundError(
            f"dataset is missing required samples: {', '.join(missing_samples)}"
        )
