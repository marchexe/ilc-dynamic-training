#!/usr/bin/env python3
"""write_canonical_outputs(): the top-level orchestrator that ties every
other pbt/reporting/ submodule together into one run-directory artifact
set. This is the highest-level module in the subpackage -- it is the only
one allowed to import from all the others."""

import copy
import json
from pathlib import Path

from training.pbt.reporting.constants import EVENTS_NAME, METRICS_NAME, REPORT_NAME, SUMMARY_NAME
from training.pbt.reporting.io import ensure_run_layout, read_events, write_resolved_config
from training.pbt.reporting.markdown_report import write_report, write_summary_json
from training.pbt.reporting.metrics_rows import (
    refresh_metrics_csv,
    write_exploit_table,
    write_skipped_exploits_table,
    write_tiered_metrics_csv,
)
from training.pbt.reporting.plots import write_existing_physics_reports
from training.pbt.reporting.report_plots import write_report_plots

def write_canonical_outputs(run_dir, manifest):
    """Write derived artifacts without changing the caller or training manifest.

    Legacy report renderers consume a manifest-shaped view. Enrich a private
    snapshot and persist report metadata in summary.json, never manifest.json.
    """
    report_view = copy.deepcopy(manifest)
    ensure_run_layout(run_dir)
    run_dir = Path(run_dir)
    write_resolved_config(run_dir, report_view.get("config", {}))
    refresh_metrics_csv(run_dir, report_view)
    physics_outputs = write_existing_physics_reports(run_dir, report_view)
    report_view["checkpoint_selection_for_report"] = physics_outputs["checkpoint_selection_metadata"]
    tiered_metrics_path = write_tiered_metrics_csv(run_dir, report_view)
    events = read_events(run_dir)
    exploit_table = write_exploit_table(run_dir, events)
    skipped_exploit_table = write_skipped_exploits_table(run_dir, events)
    report_plots = write_report_plots(run_dir, report_view)
    report_view["canonical_artifacts"] = {
        "events": str(run_dir / EVENTS_NAME),
        "metrics": str(run_dir / METRICS_NAME),
        "tiered_metrics": str(tiered_metrics_path),
        "summary": str(run_dir / SUMMARY_NAME),
        "report": str(run_dir / REPORT_NAME),
        # Heterogeneous by design: report_plots's values are result dicts
        # ({"png", "warnings", "generations", "members", "metric_keys"});
        # physics_outputs's values (and the two table csv paths) are bare
        # path strings. Nothing downstream depends on uniform shape here --
        # build_summary() reconstructs summary["plots"] independently from
        # REPORT_PLOT_NAMES + fixed paths, never reads this dict directly.
        "plots": {
            **report_plots,
            **physics_outputs,
            "exploit_table_csv": str(exploit_table),
            "skipped_exploit_table_csv": str(skipped_exploit_table),
        },
        "resolved_config": str(run_dir / "resolved_config.yaml"),
    }
    summary_path = write_summary_json(run_dir, report_view)
    summary = json.loads(summary_path.read_text(encoding="utf-8"))
    report_path = write_report(run_dir, report_view, summary)
    report_view["canonical_artifacts"]["report"] = str(report_path)
    return report_view["canonical_artifacts"]
