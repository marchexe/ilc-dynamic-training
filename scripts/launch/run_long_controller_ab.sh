#!/usr/bin/env bash
# Launch both legs of the long controller-off/controller-active A/B
...
#   nohup bash scripts/launch/run_long_controller_ab.sh > /tmp/ab_wrapper.log 2>&1 &
...
EXP_OFF="pretrained_pbt_4gpu_long_controller_off_${TIMESTAMP}"
EXP_ACTIVE="pretrained_pbt_4gpu_long_controller_active_${TIMESTAMP}"
...
nohup bash "$LAUNCH_DIR/run_long_controller_off.sh" "$EXP_OFF" >"$LOG_OFF" 2>&1 &
PID_OFF=$!
disown "$PID_OFF"
echo "$PID_OFF" > "$PID_FILE_OFF"

nohup bash "$LAUNCH_DIR/run_long_controller_active.sh" "$EXP_ACTIVE" >"$LOG_ACTIVE" 2>&1 &
PID_ACTIVE=$!
disown "$PID_ACTIVE"
...
This script did not wait for either to finish.
SUMMARY
