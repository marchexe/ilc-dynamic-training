...
def _set_random_seed(seed, local_rank=None):
    if seed is None:
        return None
    effective_seed = int(seed) + (0 if local_rank is None else int(local_rank))
...
def train_load(args):
    ...
    if args.seed is not None:
        train_generator = torch.Generator().manual_seed(args.seed + rank_offset + 1)
        val_generator = torch.Generator().manual_seed(args.seed + rank_offset + 2)
...
def _main(args):
    ...
    if training_mode:
        ...
        if args.training_controller is not None:
            if args.lr_scheduler != "none":
                raise ValueError("--training-controller currently requires --lr-scheduler none")
            from weaver.utils.training_control import build_training_controller

            training_controller = build_training_controller(
                args.training_controller,
                opt,
                default_log_path=args.model_prefix + "_controller.jsonl",
                steps_per_epoch=args.steps_per_epoch,
            )
            if args.load_epoch is not None:
