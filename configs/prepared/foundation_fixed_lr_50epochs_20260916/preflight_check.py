...
root=Path.cwd()
review=root/'configs/prepared/foundation_fixed_lr_50epochs_20260916'
smoke = len(sys.argv) > 1 and sys.argv[1] == 'smoke'
if smoke:
 review=review/'smoke'
review.mkdir(parents=True, exist_ok=True)
args=SimpleNamespace(config=root/('configs/experiments/foundation_fixed_lr_smoke.yaml' if smoke else 'configs/experiments/foundation_fixed_lr_50epochs.yaml'), experiment_name=None,
...
assert shared['generations']==(1 if smoke else 50) and shared['weaver_epochs_per_generation']==1
...
smoke_manifest=json.loads((root/'runs/pbt/foundation_fixed_lr_smoke_20260916/study/manifest.json').read_text())
assert smoke_manifest['status']=='completed'
smoke_config=smoke_manifest['config']
assert shared == dict(smoke_config['shared'], generations=50)
smoke_pbt = dict(smoke_config['pbt'], exploit_interval_generations=51)
# The completed manifest contains measured baseline bookkeeping added by the runner.
smoke_pbt['baseline_metric_value'] = smoke_pbt.pop('configured_baseline_metric_value')
smoke_pbt.pop('runtime_baseline_metric_value')
if 'baseline_metric_value' not in pbt:
 assert smoke_pbt.pop('baseline_metric_value') is None
assert pbt == smoke_pbt
assert c['population']==smoke_config['population']
assert shared['generations']==50 and shared['initial_epoch']==17
assert not shared.get('freeze_model_weights') and not shared.get('initial_controller')
